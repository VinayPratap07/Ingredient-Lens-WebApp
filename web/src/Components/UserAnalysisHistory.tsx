...
export interface AnalysisRecord {
  ...
  createdAt: string;
  updatedAt?: string;
  Analysis: RawIngredient[] | { message?: string };
  __v?: number;
}
    ...
    <div className="bg-[#F7F3EA] rounded-2xl border border-[#C8D0CA] p-6 shadow-sm font-sans text-[#18201C] space-y-4 max-w-xl mx-auto">
      ...
      <ul className="divide-y divide-[#C8D0CA]/50">
        {analysis.map((item) => {
          ...
          const ingredients: RawIngredient[] = isCompleted
            ? (item.Analysis as RawIngredient[])
            : [];
          ...
          const errorMessage =
            ...
            item.Analysis !== null
              ? (item.Analysis as { message?: string }).message ||
                "Analysis run failed"
              : "Analysis run failed";
          ...
          return (
            ...
            >
              <div className="space-y-2 min-w-0">
                ...
                {/* Sample Badges or Error Message */}
                {isCompleted ? (
              ...
              <Link to={`/image-analysis/${item._id}`}>
                {/* View Arrow */}
